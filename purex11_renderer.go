...
func NewPureX11Renderer(host *PureX11Host, face font.Face) *PureX11Renderer {
	return &PureX11Renderer{
		host:       host,
		face:       face,
...
func (r *PureX11Renderer) SetPalette(pal *[256]uint32) {
	// X11 renderer uses TrueColor naturally
}

func (r *PureX11Renderer) SetCursor(x, y int, visible bool, shape CursorShape) {
...
func (r *PureX11Renderer) getCellColors(cell CharInfo) (uint32, uint32) {
...
func (r *PureX11Renderer) Render(buf, shadow []CharInfo, w, h int, forceRedraw bool) {
	start := time.Now()
	r.host.mu.Lock()
...
func (r *PureX11Renderer) drawCachedGlyph(img *image.RGBA, char rune, px, py, rw int, fg, bg uint32, fgCol, bgCol color.RGBA) {
...
func (r *PureX11Renderer) Flush() {
	start := time.Now()
	calls := r.host.flushImage()

	// В XGB нет метода Sync(). Буфер команд сбрасывается автоматически,
	// когда основной цикл вызывает WaitForEvent().
...
func (r *PureX11Renderer) reportStats() {
...
func (r *PureX11Renderer) drawCustomChar(img *image.RGBA, char rune, px, py, cw, ch int, col color.Color) bool {
	mx := px + cw/2
	my := py + ch/2
	thick := r.host.scale
